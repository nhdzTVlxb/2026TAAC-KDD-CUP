...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--seq_causal', action='store_true', default=False,
                        help='Whether the LongerEncoder self-attention uses a causal mask '
                             '(only effective when --seq_encoder_type=longer)')
    parser.add_argument('--seq_interest_ratios', type=str, default='1.0,0.7,0.1',
                        help='Comma-separated sequence prefix ratios used to build '
                             'multiple interest views per sequence domain')
    ...
    parser.add_argument('--user_dense_pair_mode', type=str, default='new',
                        choices=['old', 'new', 'none'],
                        help='User dense/int pair implementation: '
                             'old = legacy single user_dense token plus pair residual, '
                             'new = aligned multi-token user_dense tokenizer, '
                             'none = plain single user_dense token without pair logic')
    parser.add_argument('--user_dense_tokenizer_type', type=str, default='aligned',
                        choices=['global', 'aligned'],
                        help='How to turn user_dense features into token(s): '
                             'global = one pooled token, '
                             'aligned = per-feature summaries with special int+dense alignment for selected fids')
    parser.add_argument('--aligned_user_dense_fids', type=str, default='62,63,64,65,66',
                        help='Comma-separated user feature ids treated as high-confidence int+dense aligned pairs')
    parser.add_argument('--independent_user_dense_fids', type=str, default='61,87',
                        help='Comma-separated high-dimensional user_dense fids that become independent tokens')
    parser.add_argument('--enable_user_dense_smooth_norm', action='store_true', default=False,
                        help='Apply smooth-clip + learnable affine to selected user_dense features before projection')
    parser.add_argument('--user_dense_smooth_norm_fids', type=str, default='62,63,64,65,66',
                        help='Comma-separated user_dense fids that use smooth-clip normalization')
    parser.add_argument('--excluded_user_dense_fids', type=str, default='',
                        help='Comma-separated user dense fids to exclude from dense projection (e.g., "87")')

    # Excluded user int fids (will not be embedded)
    parser.add_argument('--excluded_user_int_fids', type=str, default='',
                        help='Comma-separated user int fids to exclude from embedding (e.g., "89,90,91")')
...
def main() -> None:
    ...
    model_args = {
        ...
        "seq_interest_ratios": args.seq_interest_ratios,
        ...
        "user_dense_pair_mode": args.user_dense_pair_mode,
        "user_dense_tokenizer_type": args.user_dense_tokenizer_type,
        "aligned_user_dense_fids": args.aligned_user_dense_fids,
        "independent_user_dense_fids": args.independent_user_dense_fids,
        "enable_user_dense_smooth_norm": args.enable_user_dense_smooth_norm,
        "user_dense_smooth_norm_fids": args.user_dense_smooth_norm_fids,
        "excluded_user_dense_fids": args.excluded_user_dense_fids,
        "excluded_user_int_fids": args.excluded_user_int_fids,
    ...
    num_base_sequences = len(pcvr_dataset.seq_domains)
    num_sequences = model.num_sequences
    num_ns = model.num_ns
    T = args.num_queries * num_sequences + num_ns
    logging.info(
        f"PCVRHyFormer model created: base_sequences={num_base_sequences}, "
        f"seq_interest_ratios={model.seq_interest_ratios}, "
        f"sequence_slots={num_sequences}, num_ns={num_ns}, T={T}, "
