...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        reset_sparse_each_epoch: bool = False,
        ...
        self.reset_sparse_each_epoch: bool = bool(reset_sparse_each_epoch)
    ...
    def _save_epoch_checkpoint(
        ...
        """Save an epoch-end checkpoint, preserving EMA weights when enabled."""
        parts = [f"epoch{epoch}", f"global_step{global_step}"]
        for key in ("layer", "head", "hidden"):
            if key in self.ckpt_params:
                parts.append(f"{key}={self.ckpt_params[key]}")
        ckpt_dir = os.path.join(self.save_dir, ".".join(parts))
        os.makedirs(ckpt_dir, exist_ok=True)
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.num_epochs + 1):
            # ─── Mega7: ID Mask Cold Start Exercise warmup (port from 0.825) ──
            # epoch 1: no mask (model needs at least 1 pass on real IDs).
            # epoch N: linearly ramp to min(0.1, N*0.01). Same schedule as
            # the upstream teammate-0.825 trainer.py:388-393.
            if epoch == 1:
                mask_p = 0.0
            else:
                mask_p = min(0.1, epoch * 0.01)
            if hasattr(self.model, 'mask_prob'):
                self.model.mask_prob = mask_p
                if hasattr(self.model, 'user_ns_tokenizer'):
                    self.model.user_ns_tokenizer.mask_prob = mask_p
                if hasattr(self.model, 'item_ns_tokenizer'):
                    self.model.item_ns_tokenizer.mask_prob = mask_p
            logging.info(f"Epoch {epoch}: ID mask Cold Start Exercise mask_prob={mask_p:.3f}")
            ...
            self._handle_validation_result(total_step, val_auc, val_logloss)
            self._save_epoch_checkpoint(epoch, total_step, val_auc, val_logloss)
            ...
            if epoch >= self.reinit_sparse_after_epoch and self.sparse_optimizer is not None:
                ...
                reinit_ptrs = self.model.reinit_high_cardinality_params(
                    ...
                    reset_all=self.reset_sparse_each_epoch,
                )
                if self.ema is not None:
    ...
    def _make_model_input(self, device_batch: Dict[str, Any]) -> ModelInput:
        ...
        seq_time_ts: Dict[str, torch.Tensor] = {}   # Mega8 Fourier per-seq-item raw ts
        ...
        for domain in seq_domains:
            ...
            seq_time_ts[domain] = device_batch.get(
                ...
                torch.zeros(B, L, dtype=torch.long, device=self.device))
            # EXPΩ Patch 1: 8d sincos features per event (float32).
            seq_per_event_unified_sincos[domain] = device_batch.get(
                f'{domain}_per_event_unified_sincos',
                torch.zeros(B, L, 8, dtype=torch.float, device=self.device))
            # EXPΩ Patch 2: session bucket triples (long).
        ...
        return ModelInput(
            ...
            time_match_feats=device_batch.get(
                'time_match_feats',
                torch.zeros(B0, 0, dtype=torch.float, device=self.device)),
            timestamp=device_batch.get(
                'timestamp',
                torch.zeros(B0, dtype=torch.long, device=self.device)),
        )
